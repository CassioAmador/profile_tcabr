...
if len(shot.freqs_over)!=0:
    ax1.axhline(rf.freq2den(shot.freqs_over[0]*1e9),color='k',linestyle='-.')
    ax1.axhline(rf.freq2den(shot.freqs_over[-1]*1e9),color='k',linestyle='-.')
else:
    ax1.axhline(rf.freq2den(shot.X['K'][-1]*1e9),color='r',linestyle='-.')
    ax1.axhline(rf.freq2den(shot.X['Ka'][0]*1e9),color='r',linestyle='-.')

ax2=fig.add_subplot(211)
lined, = ax2.plot(abs(pos_abel[:-1]-shot.pos) * 1e2)
